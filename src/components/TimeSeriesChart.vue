...
const updateChart = () => {
  ...
  checkedOptions.value.forEach(algorithm => {
    ...
    const transparentColor = color.replace('rgb', 'rgba').replace(')', ', 0.8)')
...
const updateView = () => {
  ...
  myChart.dispatchAction({ type: 'dataZoom', startValue: Math.max(targetX - ZOOM_WIDTH / 2, data[0][0]), endValue: targetX + ZOOM_WIDTH / 2 })
}
